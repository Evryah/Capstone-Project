...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Check if in preview mode (e.g., via a URL query parameter or specific condition)
    const isPreviewMode = new URLSearchParams(window.location.search).has('preview');

    if (isPreviewMode) {
        sendRequestButton.style.display = 'none'; // Hide the "Send Request" button
    }
...
fetch(`${API_BASE_URL}/machines/owner/${machine.ownerId}/listings?exclude=${machineId}`)
    ...
    .catch(error => {
       ...
       console.log("Owner Full Address:", ownerFullAddress);
            ...
            window.location.href = 'Renter/RBookingDetails.html';
        }
