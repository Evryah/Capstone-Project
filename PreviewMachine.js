document.addEventListener('DOMContentLoaded', function () {
    ...
    if (!machineId) {
        console.error('Machine ID is missing from the URL.');
        alert('Invalid machine ID. Redirecting to home.');
        window.location.href = 'home.html'; // Redirect to a safe page
        return;
    }
...
fetch(`${API_BASE_URL}/machines/owner/${machine.ownerId}/listings?exclude=${machineId}`)
    ...
    .then(listings => {
        ...
        if (listings && listings.length > 0) {
            listings.forEach(listing => {
                ...
                cardLink.href = `/machine_detail.html?id=${listing.id}`;
    ...
    .catch(error => {
      ...
      // Updated booking request handler with toast notifications
function handleBookingRequest() {
    ...
    if (isNaN(endDate.getTime())) {
        showToast('Please select a valid end date.');
        return;
    }
    ...
    const machineTitle = document.getElementById('machine-title').textContent || 'Unknown Machine';
    const ownerName = document.getElementById('owner-name').textContent || 'Unknown Owner';
    const ownerPhone = document.getElementById('location').textContent || 'No contact info';

    // Safely get owner address details
    const ownerBarangay = document.getElementById('owner-barangay')?.textContent || '';
    ...
    const machineCategory = document.getElementById('machine-category').textContent || 'Uncategorized';
    const machineId = new URLSearchParams(window.location.search).get('id');

    const userId = localStorage.getItem('userId');
    if (!userId) {
        showToast('User ID not found. Please log in or register.');
        return;
    }

    // Dummy renter details for now
    const renterFirstName = "Ping"; // Replace with dynamic input
    const renterLastName = "Pong"; // Replace with dynamic input
    const renterAddress = "Lorem Ipsum Generator"; // Replace with dynamic input
    const renterPhone = "09264906925"; // Replace with dynamic input
    ...
    window.location.href = `Renter/RBookingDetails.html?bookingId=${bookingId}`;
}
...
const authButtons = document.querySelector('.auth-buttons'); // Wrapper for Login and Register buttons

// Clone auth buttons to insert dynamically into the menu
const loginRegisterContainer = document.createElement('div');
loginRegisterContainer.id = 'auth-buttons-mobile';
loginRegisterContainer.classList.add('mobile-auth-buttons');
loginRegisterContainer.innerHTML = authButtons.innerHTML; // Clone buttons

// Append cloned buttons to the nav-links container
navLinks.appendChild(loginRegisterContainer);

// Toggle the mobile menu and show/hide login and register buttons dynamically
menuIcon.addEventListener('click', function () {
    navLinks.classList.toggle('active');

    // Show/Hide login and register buttons dynamically
    const mobileAuthButtons = document.getElementById('auth-buttons-mobile');
    if (navLinks.classList.contains('active')) {
        mobileAuthButtons.style.display = 'flex'; // Show buttons
    } else {
        mobileAuthButtons.style.display = 'none'; // Hide buttons
    }
});

// Ensure the buttons are hidden by default when the menu is not active
